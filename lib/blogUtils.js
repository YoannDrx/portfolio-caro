...
export async function getSortedPostsData() { // Notez que la fonction est maintenant asynchrone
  const fileNames = fs.readdirSync(postsDirectory);
  const allPostsData = await Promise.all(fileNames.map(async (fileName) => { // Utilisation de Promise.all et async
    ...
    return {
      id,
      contentHtml, // Ajout du contenu HTML
      ...matterResult.data,
    };
  }));
