...
function parseExpertiseMetadataFromContent(content: string): {
  ...
  try {
    ...
    if (frontmatter.productionCompanies && Array.isArray(frontmatter.productionCompanies)) {
 ...
 */
async function getDocumentairesFromPrisma(locale: Locale): Promise<Documentaire[]> {
  try {
    ...
    return works.map((work) => {
      ...
      const productionCompanies =
        work.productionCompanySlugs && Array.isArray(work.productionCompanySlugs)
...
export const getExpertise = cache(
  async (slug: string, locale: Locale): Promise<Expertise | null> => {
    try {
      ...
      const allImages = expertise.images.map((img) => assetPathToImageSrc(img.path))
...
function splitMarkdownIntoSections(markdownContent: string): string[] {
  ...
  return markdownContent
    ...
    .map((section) =>
      section.replace('<!-- section:start -->', '').replace('<!-- section:end -->', '').trim()
    )
 ...
 */
export function getSectionLayout(expertise: Expertise, sectionIndex: number): SectionLayout {
